import React from "react";

export const SearchSvg = () => (
  <svg
    width="1.5rem"
    height="1.5rem"
    className="search-icon"
    viewBox="0 0 24 24"
  >
    <path
      d="M9.5 3A6.5 6.5 0 0 1 16 9.5c0 1.61-.59 3.09-1.56 4.23l.27.27h.79l5 5l-1.5 1.5l-5-5v-.79l-.27-.27A6.516 6.516 0 0 1 9.5 16A6.5 6.5 0 0 1 3 9.5A6.5 6.5 0 0 1 9.5 3m0 2C7 5 5 7 5 9.5S7 14 9.5 14S14 12 14 9.5S12 5 9.5 5z"
      fill="currentColor"
    ></path>
  </svg>
);

export const CheckSvg = () => (
  <svg width="1rem" height="1rem" viewBox="0 0 24 24" className="verified-icon">
    <path
      d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10s10-4.5 10-10S17.5 2 12 2m-2 15l-5-5l1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"
      fill="currentColor"
    ></path>
  </svg>
);

export const BookmarkOutlinedSvg = () => (
  <svg
    width="1.6rem"
    height="1.6rem"
    viewBox="0 0 24 24"
    className="bookmark-outlined-icon"
  >
    <path
      d="M17 18l-5-2.18L7 18V5h10m0-2H7a2 2 0 0 0-2 2v16l7-3l7 3V5a2 2 0 0 0-2-2z"
      fill="currentColor"
    ></path>
  </svg>
);

export const BookmarkSvg = () => (
  <svg
    width="1.6rem"
    height="1.6rem"
    viewBox="0 0 24 24"
    className="bookmark-icon"
  >
    <path
      d="M17 3H7a2 2 0 0 0-2 2v16l7-3l7 3V5a2 2 0 0 0-2-2z"
      fill="currentColor"
    ></path>
  </svg>
);

export const WatchLaterOutlinedSvg = () => (
  <svg
    width="1.6rem"
    height="1.6rem"
    viewBox="0 0 24 24"
    className="clock-outline-icon"
  >
    <path
      d="M12 20a8 8 0 0 0 8-8a8 8 0 0 0-8-8a8 8 0 0 0-8 8a8 8 0 0 0 8 8m0-18a10 10 0 0 1 10 10a10 10 0 0 1-10 10C6.47 22 2 17.5 2 12A10 10 0 0 1 12 2m.5 5v5.25l4.5 2.67l-.75 1.23L11 13V7h1.5z"
      fill="currentColor"
    ></path>
  </svg>
);

export const WatchLaterSvg = () => (
  <svg
    width="1.6rem"
    height="1.6rem"
    viewBox="0 0 24 24"
    className="clock-icon"
  >
    <path
      d="M12 2A10 10 0 0 0 2 12a10 10 0 0 0 10 10a10 10 0 0 0 10-10A10 10 0 0 0 12 2m4.2 14.2L11 13V7h1.5v5.2l4.5 2.7l-.8 1.3z"
      fill="currentColor"
    ></path>
  </svg>
);

export const HeartOutlinedSvg = () => (
  <svg
    width="1.6rem"
    height="1.6rem"
    viewBox="0 0 24 24"
    className="heart-outline-svg"
  >
    <path
      d="M12.1 18.55l-.1.1l-.11-.1C7.14 14.24 4 11.39 4 8.5C4 6.5 5.5 5 7.5 5c1.54 0 3.04 1 3.57 2.36h1.86C13.46 6 14.96 5 16.5 5c2 0 3.5 1.5 3.5 3.5c0 2.89-3.14 5.74-7.9 10.05M16.5 3c-1.74 0-3.41.81-4.5 2.08C10.91 3.81 9.24 3 7.5 3C4.42 3 2 5.41 2 8.5c0 3.77 3.4 6.86 8.55 11.53L12 21.35l1.45-1.32C18.6 15.36 22 12.27 22 8.5C22 5.41 19.58 3 16.5 3z"
      fill="currentColor"
    ></path>
  </svg>
);

export const HeartSvg = () => (
  <svg width="1.6rem" height="1.6rem" viewBox="0 0 24 24" className="heart-svg">
    <path
      d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.27 2 8.5C2 5.41 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.08C13.09 3.81 14.76 3 16.5 3C19.58 3 22 5.41 22 8.5c0 3.77-3.4 6.86-8.55 11.53L12 21.35z"
      fill="currentColor"
    ></path>
  </svg>
);

export const PlaylistPlusSvg = () => (
  <svg
    width="1.6rem"
    height="1.6rem"
    viewBox="0 0 24 24"
    className="playlist-plus-svg"
  >
    <path
      d="M2 16h8v-2H2m16 0v-4h-2v4h-4v2h4v4h2v-4h4v-2m-8-8H2v2h12m0 2H2v2h12v-2z"
      fill="currentColor"
    ></path>
  </svg>
);

export const PencilSvg = () => (
  <svg width="1.6rem" height="1.6rem" viewBox="0 0 24 24" className="edit-svg">
    <path
      d="M20.71 7.04c.39-.39.39-1.04 0-1.41l-2.34-2.34c-.37-.39-1.02-.39-1.41 0l-1.84 1.83l3.75 3.75M3 17.25V21h3.75L17.81 9.93l-3.75-3.75L3 17.25z"
      fill="currentColor"
    ></path>
  </svg>
);

export const DeleteOutlineSvg = () => (
  <svg
    width="1.6rem"
    height="1.6rem"
    viewBox="0 0 24 24"
    className="delete-svg"
  >
    <path
      d="M6 19a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V7H6v12M8 9h8v10H8V9m7.5-5l-1-1h-5l-1 1H5v2h14V4h-3.5z"
      fill="currentColor"
    ></path>
  </svg>
);

export const DeleteSvg = () => (
  <svg width="1.3rem" height="1.3rem" viewBox="0 0 24 24">
    <path
      d="M19 4h-3.5l-1-1h-5l-1 1H5v2h14M6 19a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V7H6v12z"
      fill="currentColor"
    ></path>
  </svg>
);

export const CloseSvg = () => (
  <svg width="1.5rem" height="1.5em" viewBox="0 0 24 24">
    <path
      d="M19 6.41L17.59 5L12 10.59L6.41 5L5 6.41L10.59 12L5 17.59L6.41 19L12 13.41L17.59 19L19 17.59L13.41 12L19 6.41z"
      fill="currentColor"
    ></path>
  </svg>
);

export const AddSvg = () => (
  <svg width="1.6rem" height="1.6rem" viewBox="0 0 24 24">
    <path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" fill="currentColor"></path>
  </svg>
);

export const ProfileSvg = () => (
  <svg width="1.5rem" height="1.5rem" viewBox="0 0 24 24">
    <path
      d="M12 4a4 4 0 0 1 4 4a4 4 0 0 1-4 4a4 4 0 0 1-4-4a4 4 0 0 1 4-4m0 10c4.42 0 8 1.79 8 4v2H4v-2c0-2.21 3.58-4 8-4z"
      fill="currentColor"
    ></path>
  </svg>
);

export const ShareSvg = () => (
  <svg width="1.6rem" height="1.6rem" viewBox="0 0 24 24">
    <path
      d="M21 12l-7-7v4C7 10 4 15 3 20c2.5-3.5 6-5.1 11-5.1V19l7-7z"
      fill="currentColor"
    ></path>
  </svg>
);
